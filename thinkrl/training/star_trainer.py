...
class STaRTrainer:
    ...
    def generate_and_filter(self, prompts: list[str], targets: list[str] = None):
        ...
        for i, r in enumerate(rewards):
            if r > 0.5: # Correct
                ...
                if targets: failed_targets.append(targets[i])
    ...
    def fine_tune(self, collected_data: list[dict], iter_idx: int):
        ...
        for step in range(steps):
            # Sample a batch from collected_data
            indices = torch.randint(0, len(collected_data), (4,)) # Batch size 4
            batch = []
            max_len = 0
            for idx in indices:
