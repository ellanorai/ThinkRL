...
class DAPOConfig(AlgorithmConfig):
    ...
    def _validate_dapo_config(self) -> None:
        ...
        if self.clip_ratio_lower <= 0:
            raise ValueError(f"clip_ratio_lower must be positive, got {self.clip_ratio_lower}")
        ...
        if self.overlong_penalty < 0:
            raise ValueError(f"overlong_penalty must be non-negative, got {self.overlong_penalty}")
...
class DAPOAdvantageEstimator:
    ...
    def compute_advantages(
        ...
        if self.config.truncation_reward_adjustment and sequence_lengths is not None:
            rewards = self._apply_overlong_penalty(rewards, sequence_lengths, attention_mask)
        ...
        if self.config.use_group_relative_advantage:
            advantages = self._compute_group_relative_advantages(returns, attention_mask)
        else:
    ...
    def _apply_overlong_penalty(
        ...
        if penalty_mask.any():
            ...
            for i, (is_overlong, seq_len) in enumerate(zip(penalty_mask, sequence_lengths)):
                if is_overlong:
    ...
    def _compute_gae_returns(
        ...
        for i in range(batch_size):
            ...
            for t in reversed(range(len(seq_rewards))):
                running_return = seq_rewards[t] + self.config.gae_lambda * running_return
...
class DAPOLoss:
    ...
    def compute_policy_loss(
        ...
        policy_loss = torch.where(use_higher_mask, policy_loss_higher, policy_loss_standard)
        ...
        if self.config.token_level_loss:
            ...
            # Sequence-level loss
            sequence_losses = (policy_loss * attention_mask).sum(dim=1) / attention_mask.sum(dim=1).clamp(min=1)
            loss = sequence_losses.mean()
        
        # Compute metrics
        metrics = self._compute_policy_metrics(ratio, advantages, attention_mask, use_higher_mask)
...
class DAPO(BaseAlgorithm):
    ...
    def __init__(self, config: DAPOConfig):
        ...
        logger.info(f"Initialized DAPO with clip_ratio_lower={config.clip_ratio_lower}, "
    ...
    def step(self, batch: Dict[str, Any], **kwargs) -> AlgorithmOutput:
        ...
        if algorithm_output.loss is not None:
            ...
            if self.config.dynamic_sampling and self._step_count > 0:
                ...
                gradients = [p.grad for p in self._model.parameters() if p.grad is not None]
                filtered_batch, sample_mask = self.sampler.filter_samples(batch, gradients)
