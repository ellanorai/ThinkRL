...
class TestModelProtocol:
    ...
    def test_protocol_methods_work(self):
        ...
        assert all(isinstance(p, torch.Tensor) for p in params), "All parameters should be tensors"
...
class TestMockModel:
    ...
    def test_generation(self, model):
        ...
        # Check output shape
        assert generated.shape == (2, 20), f"Generated shape {generated.shape} != (2, 20)"
    ...
    def test_train_eval_modes(self, model):
        ...
        for module in model.modules():
            if hasattr(module, 'training'):
                assert module.training is True, f"Module {type(module)} not in training mode"
        ...
        for module in model.modules():
            if hasattr(module, 'training'):
                assert module.training is False, f"Module {type(module)} not in eval mode"
