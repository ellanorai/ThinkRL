...
class TestDAPOAdvantageEstimator:
    ...
    def test_group_relative_advantages(self, config):
        ...
        assert group1_advantages > 0, f"Group 1 advantages should be positive, got {group1_advantages}"
        
        # Second group should have negative advantages
        group2_advantages = advantages[4:].mean()
        assert group2_advantages < 0, f"Group 2 advantages should be negative, got {group2_advantages}"
    ...
    def test_attention_mask_handling(self, estimator):
        ...
        assert torch.allclose(
            masked_advantages, 
            torch.zeros_like(masked_advantages),
            atol=1e-6
        ), "Masked positions should have zero advantages"
        ...
        assert not torch.allclose(
            unmasked_advantages, 
            torch.zeros_like(unmasked_advantages),
            atol=1e-6
...
class TestDAPOLoss:
    ...
    def test_policy_loss_computation(self, loss_computer):
        ...
        expected_metrics = [
            'ratio_mean', 'ratio_std', 'clipped_lower_frac',
            'clipped_upper_frac', 'clipped_higher_frac',
    ...
    def test_clip_higher_mechanism(self, config):
        ...
        assert metrics['ratio_mean'] > config.clip_ratio_higher, \
            f"Ratio {metrics['ratio_mean']} should exceed clip_ratio_higher {config.clip_ratio_higher}"
        
        assert metrics['clipped_higher_frac'] > 0, \
...
class TestDAPOSampler:
    ...
    def test_adaptive_threshold_behavior(self, sampler):
        ...
        assert 1e-8 <= final_threshold <= 1e-3, \
    ...
    def test_efficiency_target_maintenance(self, config):
        ...
        assert sampler.gradient_threshold <= initial_threshold, \
...
class TestDAPOAlgorithm:
    ...
    def test_generation_functionality(self, dapo, model):
        ...
        generation_output = dapo.generate(
            input_ids=input_ids,
            max_length=16
        )
    ...
    def test_algorithm_info(self, dapo, model):
        ...
        # DAPO-specific info
        dapo_specific_keys = [
            'clip_ratio_lower', 'clip_ratio_higher', 'dynamic_sampling',
            'token_level_loss', 'group_relative_advantage', 'step_count'
        ]
