...
class AlgorithmConfig:
    ...
    def _resolve_device(self) -> None:
        ...
        if self.device.startswith("cuda") and not torch.cuda.is_available():
            warnings.warn(
                f"CUDA device '{self.device}' requested but not available. Using CPU."
            )
            self.device = "cpu"
...
class AlgorithmOutput:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.loss is not None:
            result["loss"] = (
                self.loss.item() if hasattr(self.loss, "item") else self.loss
            )
...
class BaseAlgorithm(abc.ABC):
    ...
    def __init__(self, config: AlgorithmConfig):
        if not isinstance(config, AlgorithmConfig):
            raise TypeError(
                f"config must be an instance of AlgorithmConfig, got {type(config)}"
            )
    ...
    def save_checkpoint(self, path: str, **kwargs) -> None:
        ...
        checkpoint = {
            ...
            "model_state_dict": (
                self._model.state_dict() if hasattr(self._model, "state_dict") else None
            ),
    ...
    def load_checkpoint(self, path: str, **kwargs) -> Dict[str, Any]:
        ...
        if (
            ...
            and checkpoint["model_state_dict"] is not None
        ):
...
class AlgorithmRegistry:
    ...
    def register(
        ...
        if name in cls._algorithms and not override:
            raise ValueError(
                f"Algorithm '{name}' already registered. Use override=True to replace."
            )
    ...
    @classmethod
    def create(
        cls, name: str, config: Optional[AlgorithmConfig] = None, **kwargs
    ) -> BaseAlgorithm:
...
def assert_model_output(
    ...
    assert (
        hidden_states.shape[0] == expected_batch_size
    ), "Batch size mismatch in hidden states"
    assert (
        hidden_states.shape[1] == expected_seq_len
    ), "Sequence length mismatch in hidden states"
    ...
    assert logits.dtype in [
        torch.float32,
        torch.float16,
    ], "Logits should be float tensor"
    assert hidden_states.dtype in [
        torch.float32,
        torch.float16,
    ], "Hidden states should be float tensor"
